...
class UserModel extends \CELLA\Models\BaseModel
{
	/**
	 * Users table name
	 * 
	...
	protected $allowedFields=['name','username','password','accessgroups','menuaccess','dashboardaccess',
							'autologoff','enabled','email','avatar','interface','resetkey','lastlogin'];
	
	protected $validationRules =
	 [
	 	'username'=>'required|is_unique[users.username,userid,{userid}]',
	...
	public function getLogedUserMenuAccess()
	{
		...
		{
			$arr=json_decode($arr['menuaccess'],TRUE);
			if (is_array($arr))
			{
				return $arr;
			}
	...
	public function getUsersStatus($interval=3600)
	{
		$interval=($interval/3600);
		$current=formatDate();
		$users=$this->select('name,userid,avatar,lastlogin')->where('length(lastlogin)>',0)->orderBy('lastlogin')->limit(5)->find();
		
		return $users;
	}
	
	public function getUserBasicData($usernameOrEmail)
	{
	...
	public function getUserData($username=null,$access=null,$email=null)
	{
		$data=$this->getUsersData($username,$access,$email,2,1);
		return $data;
	}
	
	public function getUsersData($username=null,$access=null,$email=null,$enabled=2,$limit=null)
	{
		$limit=$limit==null&&$limit!=FALSE?config('Pager')->perPage:$limit;
		$result=$this;
		$filters=[];
		
		//$fields=preg_filter('/^/', $this->table.'.', $this->allowedFields);
		//array_unshift($fields,$this->primaryKey);		
		//$fields[]='users_groups.enabled as group_enabled';
		$fields[]='users.*';
		$fields[]="(select GROUP_CONCAT(`users_groups`.`ugperms`) FROM `users_groups` WHERE  FIND_IN_SET (`users_groups`.`ugref`,`users`.`accessgroups`)) as 'access'";
		$result->select(implode(',',$fields));
		//$result->join('users_groups','users_groups.ugname=users.ugname','LEFT');
		...
		if ($email!=null && \CELLA\Helpers\Strings::contains($email,'@'))
		{
			if (\CELLA\Helpers\Strings::startsWith($email,'||'))
			{
	 ...
	 * @return Array
	 */
	function getForForm($field=null,$value=null,$addEmpty=FALSE,$defValue=null,$encode=TRUE)
	{
		...
		foreach ($this->where('enabled',1)->find() as $record) 
		{
			if ($encode)
			{
	...
	/**
	 * Return array with admin users details
	 * 
	 ...
	 * @return array
	 */
	function getAdminUsers($adminlevel=null,$valueField=null)
	{
		 
		 if ($adminlevel!=null)
		 {
		 	$this->where('userlevels.name',$adminlevel);
		 }else
		 {
		 	$this->where('userlevels.level >=','999');
		 }
		 if ($valueField!=null && in_array($valueField, $this->allowedFields))
		 {
		 	$this->select($valueField);
		 }else
		 {
		 	$this->select('users.*');
		 }
		 $this->join('userlevels','users.access=userlevels.level');
		 return $this->find();
	}
	
	/**
	 * Returns array with superadmin users email addresses
	 * 
	 * @return array
	 */
	...
	function getLogedUsers()
	{
		...
		{
			$arr[$value['name']]=formatDate($value['lastlogin'],FALSE,'d M Y H:i');
		}
		return $arr;
	}
	...
	function clearResetKey($id,$newPassword=null)
	{
		...
		$model=$this->set('resetkey','')->where(is_numeric($id) ? $this->primaryKey : 'username',$id);
		if ($newPassword!=null && strlen($newPassword) > 5)
		{
			$model=$model->set('password',password_hash($newPassword,PASSWORD_DEFAULT,['cost '=>12]));
		}
